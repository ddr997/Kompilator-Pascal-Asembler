...
{
       0,    32,    32,    35,    34,    40,    40,    42,    43,    45,
      58,    60,    62,    63,    68,    70,    71,    73,    92,    96,
     116,   136,   174,   181,   182,   183
};
#endif
...
static const char *const yytname[] =
{
  ...
  "T_WRITE", "T_ASSIGN", "T_INTEGER", "T_REAL", "T_MULOP", "ID", "NUM",
  "'+'", "'-'", "'('", "')'", "';'", "','", "':'", "'.'", "$accept",
  "standard_type", "type", "identifier_list", "statement", "expression",
  "start", "program", "$@1", "program_identifier_list", "declarations",
  "compound_statement", "statement_list", YY_NULLPTR
};
#endif
...
static const yytype_int16 yytoknum[] =
{
     ...
     265,   266,   267,   268,    43,    45,    40,    41,    59,    44,
      58,    46
};
# endif
...
{
       4,    -7,    29,   -39,    14,   -39,     6,   -39,   -13,    13,
      20,   -39,   -39,    30,    21,    31,   -39,    -5,    -4,   -39,
      23,    18,    22,    32,   -39,    -6,   -39,   -39,   -39,   -39,
      19,    27,     8,    24,    -4,   -39,    25,   -39,   -39,     8,
       8,    11,   -39,   -39,   -39,    33,     2,     8,     8,     8,
     -39,   -39,    33,    33
};
...
static const yytype_int8 yydefact[] =
{
       ...
       0,     0,     0,     0,    15,     0,     8,    12,    13,    11,
       0,     0,     0,     0,     0,     9,     0,    24,    25,     0,
       0,    17,    14,    16,    18,    22,     0,     0,     0,     0,
      23,    21,    19,    20
};
...
{
      33,    45,    46,    22,     9,     4,    10,     1,    23,    51,
      52,    53,    34,    47,    20,    21,    48,    49,     7,    50,
      37,    38,    47,    39,    40,    48,    49,    27,    28,     5,
       6,    11,    12,    16,    14,    26,    18,    35,    31,    36,
      32,    43,    44,     0,    47,    42
};

static const yytype_int8 yycheck[] =
{
       6,    39,    40,     7,    17,    12,    19,     3,    12,    47,
      48,    49,    18,    11,    19,    20,    14,    15,    12,    17,
      12,    13,    11,    15,    16,    14,    15,     9,    10,     0,
      16,    18,    12,    12,     4,    12,     5,    18,    16,    12,
       8,    34,    17,    -1,    11,    21
};
...
{
       0,     3,    28,    29,    12,     0,    16,    12,    31,    17,
      19,    18,    12,    32,     4,    30,    12,    25,     5,    33,
      19,    20,     7,    12,    26,    34,    12,     9,    10,    23,
      24,    16,     8,     6,    18,    18,    12,    12,    13,    15,
      16,    27,    21,    26,    17,    27,    27,    11,    14,    15,
      17,    27,    27,    27
};
...
{
       0,    22,    28,    30,    29,    31,    31,    25,    25,    32,
      32,    24,    23,    23,    33,    34,    34,    26,    26,    27,
      27,    27,    27,    27,    27,    27
};
...
#line 45 "parser.y"
                                                              ...
                                                              {
                                                                //printf("%d , %d\n", id_vector[i], i);
                                                                SYMTABLE.table[id_vector[i]].type = (yyvsp[-1].variable_type);
...
#line 73 "parser.y"
                                  {
                                  if(SYMTABLE.table[(yyvsp[-2].index)].type != SYMTABLE.table[(yyvsp[0].index)].type)
                                  {
                                    int newtemp = SYMTABLE.insert_to_table("$t", Input_type::TEMPORARY, SYMTABLE.table[(yyvsp[-2].index)].type);
                                    if(SYMTABLE.table[(yyvsp[0].index)].type == VarType::INTEGER)
                                    {
                                      gencode("inttoreal", (yyvsp[0].index), newtemp, -1);
                                      SYMTABLE.table[newtemp].value = (float) SYMTABLE.table[(yyvsp[0].index)].value;
                                    }
                                    if(SYMTABLE.table[(yyvsp[0].index)].type == VarType::REAL)
                                    {
...
#line 96 "parser.y"
                                      ...
                                      if(SYMTABLE.table[(yyvsp[-2].index)].type != SYMTABLE.table[(yyvsp[0].index)].type)
                                      {
                                        if(SYMTABLE.table[(yyvsp[-2].index)].type == VarType::INTEGER) {(yyvsp[-2].index) = type_conversion((yyvsp[-2].index));}
                                        if(SYMTABLE.table[(yyvsp[0].index)].type == VarType::INTEGER) {(yyvsp[0].index) = type_conversion((yyvsp[0].index));}
                                        newtemp = SYMTABLE.insert_to_table("$t", Input_type::TEMPORARY, VarType::REAL);
                                      }
                                      else
                                      {
                                        if (SYMTABLE.table[(yyvsp[-2].index)].type == VarType::INTEGER)
                                          newtemp = SYMTABLE.insert_to_table("$t", Input_type::TEMPORARY, VarType::INTEGER);
                                        if (SYMTABLE.table[(yyvsp[-2].index)].type == VarType::REAL)
                                          newtemp = SYMTABLE.insert_to_table("$t", Input_type::TEMPORARY, VarType::REAL);
                                      }
